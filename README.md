...
It utillises the outputs from compTableGen.py. So, to compare groups in the Accents dataset, the file for the GCP/IBM comparison ought to be placed under the subfolder Accents/GCP_IBM. 
...
It utillises the outputs from wordDropTableGen.py. So, to compare word drops, the generated csv file for IBM must be placed into the subfolder Data.
...
So, to check the non-robust for the IBM ASR, the file ought to be placed in the subfolder IBM.
