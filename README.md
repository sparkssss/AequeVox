...
In order to use the IPython notebook, the csv generated from compTableGen.py must be placed in the appropriate folder. (Under datasetType/asrPair). For instance, 
MS_GCP Differential Speech Results - Clipping.csv ought to be placed into the AequeVox/Analysis/Differential Testing Stats/Accents/MS_GCP folder. The graphs can then be generated using the Diff_Testing_Stats.ipynb located in the AequeVox/Analysis/Differential Testing Stats/Accents folder.

Alternatively, the get_file_data(asr_type, transformation_type) function in the Diff_Testing_Stats.ipynb can be modified to ensure it points to the correct file.

#### Explainability_Unfairness

It uses the Average Word Counts csv file to generate graphs for average word mispredictions.

In order to use the IPython notebook, the csv generated by wordDropTableGen.py must be placed in the appropriate folder. For instance, 
IBM Word Counts - Average.csv ought to be placed into the AequeVox/Analysis/Explainabity_Unfairness/Data folder. The graphs can then be generated using the Explainability_Graphs.ipynb located in the AequeVox/Analysis/Explainabity_Unfairness folder.

Alternatively, the get_file_data(asr_type) function in the Explainability_Graphs.ipynb can be modified to ensure it points to the correct file.
...
In order to use the IPython notebook, the csv generated by wordDropTableGen.py must be placed in the appropriate folder. For instance,
IBM Word Counts - Clipping.csv ought to be placed into the AequeVox/Analysis/IBM folder. The graphs can then be generated using the 
non-robust-words.ipynb located in the AequeVox/Analysis folder.
